...
next_btn.onclick = ()=>{
    if(que_count < musics.length - 1){ //if question count is less than total question length
...
// getting musics and options from array
function showQuetions(index){
    const que_text = document.querySelector(".que_text");

    //creating a new span and div tag for question and option and passing the value using array index
    let que_tag = '<span>'+ musics[index].numb + ". " + musics[index].question +'</span>';
    let option_tag = '<div class="option"><span>'+ musics[index].options[0] +'</span></div>'
    + '<div class="option"><span>'+ musics[index].options[1] +'</span></div>'
    + '<div class="option"><span>'+ musics[index].options[2] +'</span></div>'
    + '<div class="option"><span>'+ musics[index].options[3] +'</span></div>';
...
function optionSelected(answer){
    ...
    let correcAns = musics[que_count].answer; //getting correct answer from array
...
function showResult(){
    ...
    if (userScore > 3){ // if user scored more than 3
        ...
        let scoreTag = '<span>and congrats! 🎉, You got <p>'+ userScore +'</p> out of <p>'+ musics.length +'</p></span>';
        scoreText.innerHTML = scoreTag;  //adding new span tag inside score_Text
    }
    else if(userScore > 1){ // if user scored more than 1
        let scoreTag = '<span>and nice 😎, You got <p>'+ userScore +'</p> out of <p>'+ musics.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
    else{ // if user scored less than 1
        let scoreTag = '<span>and sorry 😐, You got only <p>'+ userScore +'</p> out of <p>'+ musics.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
}
...
function queCounter(index){
    ...
    let totalQueCounTag = '<span><p>'+ index +'</p> of <p>'+ musics.length +'</p> musics</span>';
